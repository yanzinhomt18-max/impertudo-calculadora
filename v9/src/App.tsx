...
export default function App() {
  ...
  const [preferredProductId, setPreferredProductId] = useState<string | undefined>()
  const [preferredSystemId, setPreferredSystemId] = useState<string | undefined>()

  const products = useMemo(() => {
    const q = query.trim().toLocaleLowerCase('pt-BR')
    return productDatabase.products.filter((product) => (!q || `${product.name} ${product.categoryId}`.toLocaleLowerCase('pt-BR').includes(q)) && (status === 'all' || product.technicalStatus === status))
  }, [query, status])
  const counts = useMemo(() => {
    const map = new Map<string, number>()
    for (const product of productDatabase.products) map.set(product.technicalStatus, (map.get(product.technicalStatus) ?? 0) + 1)
    return map
  }, [])
  const calculableCount = useMemo(() => productDatabase.products.filter(isProductAutoCalculable).length, [])
