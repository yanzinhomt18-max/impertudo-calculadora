...
import { buildCommercialLines, buildManualCommercialLines, calculateQuoteTotals, consolidateCalculations, packageLabel } from '../../engine/consolidation'
import { mergePricingWithCatalog } from '../../pricing/catalog'
import { useProject } from '../../project/ProjectContext'
import CommercialExtras from './CommercialExtras'
...
export default function ProjectDashboard() {
  ...
  const materialLines = useMemo(() => buildCommercialLines(materials, effectivePricing), [materials, effectivePricing])
  const manualLines = useMemo(() => buildManualCommercialLines(project.manualItems), [project.manualItems])
  const lines = useMemo(() => [...materialLines, ...manualLines], [materialLines, manualLines])
  ...
  }, [materialLines, project.pricing, applyCatalogToCurrentProject])

  function proposalText() {
    const heading = `IMPERTUDO - PROPOSTA COMERCIAL\n${project.proposalNumber}\n${project.projectName || 'Obra não informada'}`
    const meta = [project.client && `Cliente: ${project.client}`, project.location && `Local: ${project.location}`, project.consultant && `Responsável: ${project.consultant}`, project.proposalDate && `Data: ${new Date(`${project.proposalDate}T12:00:00`).toLocaleDateString('pt-BR')}`].filter(Boolean).join('\n')
    const itemText = lines.map((line, index) => `${index + 1}. ${line.productName}\n${qty(line.count)} × ${line.packageLabel} × ${money(line.unitPrice)} = ${money(line.net)}`).join('\n\n')
  ...
  function refreshPrices() {
    const applied = applyCatalogToCurrentProject(materialLines.map((line) => line.key))
  ...
  return <section className="calculatorShell projectShell">
    <div className="stepHeader"><div><div className="eyebrow dark">MODO PROJETO / OBRA</div><h2>Do cálculo à proposta</h2><p>Todos os módulos alimentam a mesma obra. Materiais, serviços e condições comerciais permanecem rastreáveis na mesma proposta.</p></div><span className="verifiedPill">salvo neste dispositivo</span></div>
    ...
    <section className="projectMetaCard">
      ...
      <div className="proposalMetaGrid"><label><span>Número da proposta</span><input value={project.proposalNumber} onChange={(e) => updateMeta({ proposalNumber: e.target.value })} placeholder="IMP-..." /></label><label><span>Data da proposta</span><input type="date" value={project.proposalDate} onChange={(e) => updateMeta({ proposalDate: e.target.value })} /></label></div>
    ...
    <section className="proposalPanel"><div className="sectionMiniHead"><h3>Orçamento / proposta</h3><span>{lines.length} linha(s)</span></div>
      <div className="proposalPriceTools"><div><strong>Tabela central de preços</strong><span>Preços de materiais são gravados como base; serviços e descontos permanecem nesta obra.</span></div><button className="secondaryButton" disabled={!materialLines.length} onClick={refreshPrices}>Atualizar materiais com a tabela atual</button>{priceStatus && <small>{priceStatus}</small>}</div>
      {materialLines.length === 0 ? <div className="contextNote">Nenhum material comercial precificado. Serviços e adicionais, se houver, continuam entrando nos totais abaixo.</div> : <div className="commercialTable">{materialLines.map((line) => <article className="commercialRow" key={line.key}><div className="commercialName"><strong>{line.productName}</strong><span>{qty(line.count)} × {line.packageLabel}</span></div><label><span>Preço unit. (R$)</span><input type="number" min="0" step="0.01" value={line.unitPrice || ''} placeholder="0,00" onChange={(e) => { const unitPrice = Math.max(0, Number(e.target.value) || 0); updatePricing(line.key, { unitPrice }); if (line.package) updateCatalogPrice({ key: line.key, productId: line.productId, productName: line.productName, packageLabel: line.packageLabel }, unitPrice) }} /></label><label><span>Tipo desconto</span><select value={line.discountType} onChange={(e) => updatePricing(line.key, { discountType: e.target.value === 'value' ? 'value' : 'pct', discountValue: 0 })}><option value="pct">%</option><option value="value">R$</option></select></label><label><span>Desconto</span><input type="number" min="0" step="0.01" value={line.discountValue || ''} placeholder="0" onChange={(e) => updatePricing(line.key, { discountValue: Math.max(0, Number(e.target.value) || 0) })} /></label><div className="commercialTotal"><span>Total líquido</span><strong>{money(line.net)}</strong>{line.discount > 0 && <small>- {money(line.discount)}</small>}</div></article>)}</div>}
